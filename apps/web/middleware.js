...
export function middleware(request) {
  try {
    ...
    if (subdomain === 'admin') {
      const targetUrl = new URL(`/admin${url.pathname === '/' ? '' : url.pathname}`, request.url);
      return NextResponse.rewrite(targetUrl);
    }

    // If a custom tenant subdomain is found (excluding app portals)
    if (subdomain && subdomain !== 'app') {
      const targetUrl = new URL(`/_tenant/${subdomain}${url.pathname === '/' ? '' : url.pathname}`, request.url);
      return NextResponse.rewrite(targetUrl);
    }
  } catch (err) {
    console.error("Middleware rewrite failed:", err);
  }
